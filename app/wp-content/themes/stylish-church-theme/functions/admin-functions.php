...
/*
churchThemes Framework Version & Theme Version

*/
function church_version(){

    $church_framework_version = "1.0.7";

    $theme_data = get_theme_data(TEMPLATEPATH . '/style.css');
    $theme_version = $theme_data['Version'];

    echo '<meta name="generator" content="'. get_option('church_themename').' '. $theme_version .'" />' ."\n";
    echo '<meta name="generator" content="church Framework Version '. $church_framework_version .'" />' ."\n";
   
}
add_action('wp_head','church_version');
...
function church_get_image($key = 'image', $width = null, $height = null, $class = "thumbnail", $quality = 90,$id = null,$link = 'src',$repeat = 1,$offset = 0,$before = '', $after = '',$single = false, $force = false, $return = false) {
    ...
    if(!empty($custom_field)) { // If the user set a custom field
        ...
        $force_all = get_option('church_force_all');
        $force_single = get_option('church_force_single');
        if($force == true OR $force_all == true OR ($force_single == true AND is_single())){  
                $set_width = '';
                $set_height = '';
        }
    
        if (get_option('church_resize') == 'true') { 
    ...
    elseif(empty($custom_field) && get_option('church_auto_img') == 'true'){
...
function church_get_embed($key, $width, $height, $class = 'video', $id = null) {
...
function church_show_pagemenu( $exclude="" ) {
    // Split the featured pages from the options, and put in an array
    if ( get_option('church_ex_featpages') ) {
        $menupages = get_option('church_featpages');
...
//Short Codes
function church_post_insert_shortcode($attr) {
...
add_shortcode('insert', 'church_post_insert_shortcode');  // use "[page]" in a post
